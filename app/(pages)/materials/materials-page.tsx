...
type RenderArgs = { locale: Locale }

export function renderMaterialsPage({ locale }: RenderArgs) {
  ...
  const fallbackFaqItems =
    faqPromoProps?.qaItems?.map((item) => ({
      question: item.q,
      answer: item.a,
    })) ?? []
  const faqSchemaItems = faqItems.length ? faqItems : fallbackFaqItems

  const faqJsonLd = faqSchemaItems.length
    ? buildFaqPageSchema({
