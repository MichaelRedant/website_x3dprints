...
type RecommendationCard = {
  label: "Aanbevolen" | "Budget alternatief" | "Premium alternatief" | "Alternatief"
...
const LOCATION_COPY: Record<LocationAnswer, string> = {
  indoor: "projecten binnenshuis",
  outdoorSeasonal: "seizoensgebonden buitengebruik",
  outdoorPermanent: "permanent buitengebruik",
}

const FUNCTIONALITY_COPY: Record<Exclude<FunctionalityAnswer, "flex">, string> = {
...
const BUDGET_COPY: Record<BudgetAnswer, string> = {
...
function priceNote(level: MaterialSuggestionRecord["economics"]["priceLevel"], budget: BudgetAnswer): string {
  const base = {
  ...
  const intent = BUDGET_COPY[budget]
  return `${base} ${intent}`
}

function buildReasons(record: MaterialSuggestionRecord, answers: SolidAnswers): string[] {
  ...
  const envReason =
    envScore >= 4
      ? `Uitstekend geschikt voor ${LOCATION_COPY[answers.location]}.`
      : envScore >= 3
        ? `Geschikt voor ${LOCATION_COPY[answers.location]}.`
        : `Alleen inzetten voor ${LOCATION_COPY[answers.location]} met bijkomende voorzorg.`

  let functionReason: string
  if (answers.functionality === "flex") {
    functionReason = "TPU is rubberachtig en veert telkens terug."
  } else {
    const descriptor = FUNCTIONALITY_COPY[answers.functionality]
    const strengthLabel = scoreLabel(
      record.mechanical.strength,
      "Zeer hoge sterkte",
      "Solide sterkte",
      "Basissterkte",
    )
    const impactLabel = scoreLabel(
      record.mechanical.impactResistance,
      "hoge impactbestendigheid",
      "goede impactbestendigheid",
      "beperkte impactbestendigheid",
    )
    functionReason = `${strengthLabel} en ${impactLabel} maken het ideaal voor ${descriptor}.`
  }
  ...
  const lookReason =
    answers.look === "noPreference"
      ...
      : matchesLook(record, answers.look)
        ? `Sluit aan bij jouw voorkeur (${steps[2].options.find((o) => o.value === answers.look)?.label ?? "specifieke look"}).`
        : null

  const bestFor = `Typische projecten: ${record.bestFor.slice(0, 3).join(", ")}.`
...
function buildRecommendationCards(answers: Answers): RecommendationCard[] | null {
    ...
    const cards: RecommendationCard[] = [
      {
        ...
        priceNote: priceNote(tpuRecord.economics.priceLevel, solidAnswers.budget),
        reasons: buildReasons(tpuRecord, solidAnswers),
      },
    ]
    if (petgRecord) {
      cards.push({
        ...
        priceNote: priceNote(petgRecord.economics.priceLevel, solidAnswers.budget),
        reasons: [
          "PETG is semi-flexibel en kan klappen opvangen zonder rubber te zijn.",
          "Geschikt als hybride aanpak: rigide body + TPU inserts.",
          `Typische projecten: ${petgRecord.bestFor.slice(0, 3).join(", ")}`,
        ],
      })
    }
    return cards
  ...
  return selection.map((record, index) => {
    const label: RecommendationCard["label"] =
      index === 0 ? "Aanbevolen" : index === 1 ? "Budget alternatief" : "Premium alternatief"
    return {
      label,
      record,
      slug: MATERIAL_SLUGS[record.materialKey] || record.id,
      priceNote: priceNote(record.economics.priceLevel, solidAnswers.budget),
      reasons: buildReasons(record, solidAnswers),
    }
  })
}
...
export default function MaterialSuggestionTool() {
  ...
  const recommendationCards = useMemo(() => buildRecommendationCards(answers), [answers])
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <GlassCard className="h-full border border-white/50 bg-white/90 p-6 shadow-lg backdrop-blur">
        ...
        <div className="mt-6 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-[0.4em] text-amber-500">Material Flow</p>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-slate-400">Klik op een nummer om een antwoord aan te passen</span>
          </div>
        </div>
      </GlassCard>
        ...
        <div className="mt-3 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-700">
          Prijsimpact: {card.priceNote}
                ...
                <p className="mt-3 text-xs text-slate-500">
                  Kleuren: {formatColors(card.record.colors)}
                </p>
                <div className="mt-4 flex flex-wrap gap-3">
                  <ShimmerButton href={contactHref}>Vraag een offerte met dit materiaal</ShimmerButton>
                  <Link
